    ...
    return (
      <form onSubmit={(event)=>{event.preventDefault(),this.props.search(this.state.searchParams)}} className="searchForm">
        <div className="searchFormField">
          ...
          <input name="searchParams" type="text" className="form-control" id="searchInput" placeholder="Search for..." value={this.state.searchParams} onChange={this._handleInput}/>
        </div>
